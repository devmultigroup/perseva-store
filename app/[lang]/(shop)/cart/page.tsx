  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <div className="mb-6 flex items-center justify-between">
        <h1 className="text-3xl font-bold">{lang === 'tr' ? 'Sepet' : 'Cart'}</h1>
        <Link className="text-sm text-gray-600 hover:underline" href={getLocalizedPath('/products', lang)}>
      ...
      {itemCount === 0 ? (
        ...
        <div className="space-y-4">
          ...
          <div className="rounded-lg border border-gray-200 bg-white">
            {items.map((it) => (
              <div key={it.id} className="flex items-center justify-between border-b p-4 last:border-b-0">
                <div>
                  <div className="font-medium">{it.product.name}</div>
                  {it.variant?.name && <div className="text-sm text-gray-600">{it.variant.name}</div>}
                </div>
