  ...
  return (
    <header className="sticky top-0 z-50 w-full border-b border-gray-200 bg-white">
      <div className="container mx-auto flex h-16 items-center justify-between px-4">
        <Link href={getLocalizedPath('/', lang)} className="text-2xl font-bold text-gray-900">
          Perseva Store
        </Link>
        ...
        <nav className="flex items-center gap-6">
          ...
          {!loading && user && (
            <form action={logout}>
              <button
                type="submit"
                className="rounded-md border border-gray-300 px-4 py-2 text-sm font-medium text-gray-900 hover:bg-gray-100 transition-colors"
              >
                Logout
              </button>
            </form>
